...
template<typename PointT>
std::vector<std::pair<Line2D,std::pair<Point2D,Point2D>>> getGroundPlaneLines(std::vector<std::vector<PointT*>>& array)
{
	...
	std::vector<std::pair<Line2D, std::pair<Point2D, Point2D>>> lines;
		...
		if(bin.size() != 0)
		{
			...
			if(linePoints.size() >= 2)
			{
				...
				if (!( std::abs(l.first) < tMax && (l.first > tSmall || std::abs(l.second) <= tYInt) && getFitRMSE(l, linePoints) < tRMSE))
				{
					...
					lines.push_back(std::make_pair(l, std::make_pair(linePoints[0], linePoints[linePoints.size()-1])));
					linePoints = {};
					c++;
					i--;
				}
			...
			{
				if(c == 0 || linePoints.size() == 0 || distancePointToLine(prototypePoint, lines[c-1].first) <= maxPointDistanceToLine)
				{
					linePoints.push_back(prototypePoint);
				}
			}
...
template<typename PointT>
std::unique_ptr<pcl::PointCloud<PointT>> groundRemoval(BinArray<PointT>& binAssignments, std::vector<std::vector<std::pair<Line2D, std::pair<Point2D,Point2D>>>> segmentLines)
{
	std::unique_ptr<pcl::PointCloud<PointT>> result(new pcl::PointCloud<PointT>);
	size_t segmentIndex = 0;
	const float distThreshold = 1.0f;
	for(auto segment : binAssignments)
	{

		for(auto pointArray : segment)
		{
			for(PointT* point : pointArray)
			{
				bool removePoint = false;
				for(auto lineInfo : segmentLines[segmentIndex])
				{
					if(std::sqrt(std::sqrt(std::pow(std::sqrt(point->x+point->y), 2)-lineInfo.second.first.first) + std::pow(point->z-lineInfo.second.first.second, 2)) <= distThreshold
					  || std::sqrt(std::sqrt(std::pow(std::sqrt(point->x+point->y), 2)-lineInfo.second.second.first) + std::pow(point->z-lineInfo.second.second.second, 2)) <= distThreshold)
					{
						removePoint = true;
						break;
					}
...
int main()
{
	...
	const unsigned int numBins = 10, numSegments = 6;
	...
	std::vector<std::vector<std::pair<Line2D,std::pair<Point2D,Point2D>>>> groundPlaneLines;

	for(int i = 0; i < binArray->size(); i++)
		groundPlaneLines.push_back(getGroundPlaneLines((*binArray)[i]));

	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud2(groundRemoval(*binArray, groundPlaneLines));
	...
	while(!viewer.wasStopped())
	{
		...
		viewer.showCloud((c1 == true ? cloud : cloud2));
		c1 = !c1;
	}
