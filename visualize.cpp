...
void doBinVisualization(const pcl::PointCloud<pcl::PointXYZ>::Ptr pPointCloud, unsigned int numBins)
{
	...
	std::unique_ptr<ground_removal::SegmentArray<pcl::PointXYZ>> pSegmentArray = ground_removal::assignPointsToBinsAndSegments<pcl::PointXYZ>(*pPointCloud, 1, numBins);
	...
	for(const ground_removal::PointArray<pcl::PointXYZ>& pointArray : (*pSegmentArray)[0])
	{
...
void doSegmentVisualization(const pcl::PointCloud<pcl::PointXYZ>::Ptr pPointCloud, unsigned int numSegments)
{
	...
	std::unique_ptr<ground_removal::SegmentArray<pcl::PointXYZ>> pSegmentArray = ground_removal::assignPointsToBinsAndSegments<pcl::PointXYZ>(*pPointCloud, numSegments, 1);
	...
	for(const ground_removal::BinArray<pcl::PointXYZ>& binArray : (*pSegmentArray))
	{
		Colour colour = colours[segmentIndex%colours.size()];
		for(const ground_removal::PointArray<pcl::PointXYZ>& pointArray : binArray)
		{
		for(const pcl::PointXYZ* pPoint : pointArray)
			{
				pcl::PointXYZRGB colouredPoint;
...
void doGroundPlaneLineVisualization(const pcl::PointCloud<pcl::PointXYZ>::Ptr pPointCloud, const std::vector<std::vector<ground_removal::Line>>& lines)
{
	...
	for(unsigned int segmentIndex = 0; segmentIndex < numSegments; segmentIndex++)
	{
		...
		unsigned int j = 0;
		for(const ground_removal::Line& l : lines[segmentIndex])
		{
...
int main(int argc, char* argv[])
{
	...
	{
		pcl::PointCloud<pcl::PointXYZ>::Ptr pCloud = readPCDFile<pcl::PointXYZ>(dataFileName);
		std::vector<std::vector<ground_removal::Line>> groundLines;
		std::unique_ptr<ground_removal::SegmentArray<pcl::PointXYZ>> pSegmentArray = ground_removal::assignPointsToBinsAndSegments(*pCloud, numSegments, numBins);
		for(const ground_removal::BinArray<pcl::PointXYZ>& binArray : *pSegmentArray)
		{
			groundLines.push_back(ground_removal::groundPlaneLinesForSegment<pcl::PointXYZ>(binArray));
		}
	...
	{
		pcl::PointCloud<pcl::PointXYZ>::Ptr pCloud = readPCDFile<pcl::PointXYZ>(dataFileName);
		std::unique_ptr<ground_removal::SegmentArray<pcl::PointXYZ>> pSegmentArray = ground_removal::assignPointsToBinsAndSegments<pcl::PointXYZ>(*pCloud, numSegments, numBins);
		pcl::PointCloud<pcl::PointXYZ>::Ptr pCloudNoGround = ground_removal::groundRemoval<pcl::PointXYZ>(*pSegmentArray);
